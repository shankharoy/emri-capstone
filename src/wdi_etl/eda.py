"""
================================================================================
Exploratory Data Analysis (EDA) Module — WDI ETL Pipeline
================================================================================
...
Description
-----------
    Standalone EDA functions for the tidy panel DataFrame produced by the pipeline.
    Usable as a library (import from wdi_etl.eda) or as a CLI script
    (python -m wdi_etl.eda).

Functions
---------
    load_panel(path)               : Load CSV or Parquet into a DataFrame
    summary_stats(df)             : Descriptive statistics per indicator
    missingness_report(df)        : Missing-value ratios per indicator
    missingness_heatmap_data(df)  : Country x Year x Indicator presence matrix
    coverage_by_country(df)       : Per-country data completeness
    coverage_by_year(df)          : Per-year observation counts
    indicator_correlation(df)     : Pairwise Pearson correlation across indicators
    distribution_by_indicator(df): Distribution metrics (skewness, kurtosis, IQR, CV)
    top_bottom(df, indicator, n)  : Top / bottom N countries for an indicator
    time_series_data(df, iso3, indicator): Country-level time series
    run_eda(path)                : Auto-run all functions + print report
